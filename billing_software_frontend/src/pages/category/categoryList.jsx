...
import { Pencil, ChevronLeft, ChevronRight, Search, Plus, Image, Video, X } from "lucide-react";
...
export default function CategoryList() {
  ...
  const toggleSpotlight = async (category) => {
    const newValue = category.spotlight_active ? 0 : 1;
    try {
      const res = await api.post("/category/toggle_spotlight", {
        id: category.id,
        spotlight_active: newValue,
      });
      if (res.data.status) {
        // Only ONE spotlight category is allowed — reset others when turning ON
        setCategories((prev) =>
          prev.map((c) =>
            c.id === category.id
              ? { ...c, spotlight_active: newValue }
              : newValue === 1
                ? { ...c, spotlight_active: 0 }
                : c
          )
        );
      } else {
  ...
  // Open preview modal
  const openPreview = (type, url, name) => {
    setPreviewModal({ isOpen: true, type, url, name });
  };

  // Close preview modal
  const closePreview = () => {
    setPreviewModal({ isOpen: false, type: null, url: null, name: null });
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f8fafc", padding: "30px", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "25px" }}>
        <div>
          <h1 style={{ fontSize: "28px", fontWeight: "800", color: "#0f172a", margin: 0 }}>Category</h1>
          <p style={{ fontSize: "14px", color: "#64748b", marginTop: "4px" }}>Manage product categories in a single screen</p>
        </div>
      </div>
              ...
              <table className="cl-table">
                <thead>
                  <tr>
                    ...
                    <th>Category Name</th>
                    <th>Status</th>
                    <th style={{ textAlign: "center" }}>Spotlight</th>
                      ...
                      <tr key={c.id}>
                        <td>
                          {c.image ? (
                            <img 
                              ...
                              onClick={() => openPreview('image', c.image, c.name)}
                              onError={(e) => {
                                e.target.style.display = 'none';
                                e.target.parentElement.innerHTML = '<span class="cl-no-media">Invalid</span>';
                              }}
                            />
                          ) : /* VIDEO DISABLED
                          c.video ? (
                            <div 
                              className="cl-video-thumb"
                              onClick={() => openPreview('video', c.video, c.name)}
                            >
                              ▶
                            </div>
                          ) : */ (
                            <span className="cl-no-media">—</span>
                          )}
                        </td>
                        ...
                        <td>
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "8px" }}>
                            <span className={`cl-status-badge ${c.spotlight_active ? "active" : "inactive"}`}>
                              {c.spotlight_active ? "ON" : "OFF"}
                            </span>
                            <label className="cl-switch-lbl" title="In the Spotlight (only one active)">
                              <input
                                type="checkbox"
                                checked={!!c.spotlight_active}
                                onChange={() => toggleSpotlight(c)}
                              />
                              <span className="cl-slider"></span>
                            </label>
                          </div>
      ...
      {/* Media Preview Modal */}
      {previewModal.isOpen && (
        <div className="cl-modal-overlay" onClick={closePreview}>
          <div className="cl-modal-content" onClick={(e) => e.stopPropagation()}>
            <button className="cl-modal-close" onClick={closePreview}>
              <X size={24} />
            </button>
            {/* VIDEO DISABLED
            {previewModal.type === 'image' ? (
              <img src={previewModal.url} alt={previewModal.name} className="cl-modal-image" />
            ) : (
              <video src={previewModal.url} className="cl-modal-video" controls autoPlay />
            )}
            */}
            <img src={previewModal.url} alt={previewModal.name} className="cl-modal-image" />
            <div className="cl-modal-label">{previewModal.name}</div>
          </div>
        </div>
      )}
    </div>
