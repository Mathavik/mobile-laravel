...
class ProductController extends Controller
{
    ...
    public function add(Request $request)
    {
        ...
        $mrp             = $request->input('mrp', null) !== null && $request->input('mrp') !== '' ? floatval($request->input('mrp')) : null;
        $model_name      = trim($request->input('model_name', ''));
        $ram             = trim($request->input('ram', ''));
        $internal_storage = trim($request->input('internal_storage', ''));
        $display_size    = trim($request->input('display_size', ''));
        $display_type    = trim($request->input('display_type', ''));
        $processor       = trim($request->input('processor', ''));
        $battery_capacity = trim($request->input('battery_capacity', ''));
        $rear_camera     = trim($request->input('rear_camera', ''));
        $front_camera    = trim($request->input('front_camera', ''));
        $operating_system = trim($request->input('operating_system', ''));
        $network_type    = trim($request->input('network_type', ''));
        $sim_slots       = trim($request->input('sim_slots', ''));
