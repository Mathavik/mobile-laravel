...
class InvoiceController extends Controller
{
    public function createInvoice(Request $request)
    {
        ...
        /* RESOLVE REAL COMPANY (prevents invoices being stored under a stale/phantom company id) */
        $companyExists = DB::table('companies')->where('id', $company_id)->where('is_deleted', 0)->exists();
        if (!$companyExists) {
            $fallback = DB::table('companies')->where('is_deleted', 0)->orderBy('id')->first();
            if (!$fallback) {
                return response()->json(["status" => false, "message" => "No valid company found"]);
            }
            Log::warning("[create_invoice] invalid company_id={$company_id}, falling back to company_id={$fallback->id}");
            $company_id = (int) $fallback->id;
        }
        ...
        DB::beginTransaction();
        try {
            /* INSERT INVOICE (invoice_no assigned from id so the sequence stays continuous) */
            ...
            $invoice_no = Invoice::numberFor($invoice->id);
            Invoice::where('id', $invoice->id)->update(['invoice_no' => $invoice_no]);
            ...
            /* DEDUCT STOCK (only for legacy invoice flow that does not already reduce stock) */
            $skipStockDeduction = !empty($request->input('skip_stock_deduction')) || !empty($request->input('source')) && $request->input('source') === 'shop_checkout';
    ...
    public function getFilteredInvoices(Request $request)
    {
        ...
        // The requested company must exist (avoids silently returning [] for a stale/phantom company id)
        $companyExists = DB::table('companies')->where('id', $company_id)->where('is_deleted', 0)->exists();
        if (!$companyExists) {
            Log::warning("[get_filtered_invoices] company_id={$company_id} does not match any active company");
        }
        ...
        // Log the exact SQL + bindings for this filtered query to aid debugging
        Log::info('[get_filtered_invoices] SQL: ' . $query->toSql(), $query->getBindings());
